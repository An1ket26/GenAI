...
from langchain_core.runnables import RunnablePassthrough,RunnableParallel,RunnableLambda,RunnableMap
...
chatHistory = []
chatHistory.append({"role":"System","content":"""You are an data engineer expert for question-answering tasks.
...
def format_chat_history(history):
    return "\n".join([
        ...
        if h['role'].lower() != 'system' else f"System Instructions: {h['content']}"
        for h in history
    ])
